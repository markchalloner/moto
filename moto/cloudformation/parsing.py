...
class ResourceMap(collections.Mapping):
    ...
    def update(self, template, parameters=None):
        ...
        for resource in resources_by_action['Add']:
        ...
        for resource in resources_by_action['Remove']:
            resource_name = resource['LogicalResourceId']
        ...
        while resources_by_action['Modify'] and tries < 5:
            for resource in resources_by_action['Modify'].copy():
                resource_name = resource['LogicalResourceId']
                resource_json = new_template[resource_name]
                try:
                    ...
                    resources_by_action['Modify'].remove(resource_name)
            tries += 1
        if tries == 5:
